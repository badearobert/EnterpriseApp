...
CASSANDRA_HOST = os.getenv('CASSANDRA_HOST', 'cassandra')  # ✅ Adaugă default value
FLASK_PORT = int(os.getenv("DATA_SERVICE_PORT", 5003))
ETCD_URL = os.getenv("ETCD_URL")

# Debug logging pentru variabile
...
def etcd_register(service_name, service_host, service_port):
    if not ETCD_URL:
        logger.warning("⚠️  ETCD_URL not configured, skipping service registration")
        return
...
class UserService(user_pb2_grpc.UserServiceServicer):
    def GetUser(self, request, context):
        ...
        query = "SELECT data FROM testks.users WHERE user_id=%s"  # ✅ Explicit keyspace
        try:
    ...
    def AddUser(self, request, context):
        ...
        user_id = request.user_id
        data = request.data
        query = "INSERT INTO testks.users (user_id, data) VALUES (%s, %s)"  # ✅ Explicit keyspace
        try:
